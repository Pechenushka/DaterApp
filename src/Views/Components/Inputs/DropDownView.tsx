...
class DropDownView extends TypedBaseComponent<dropDownViewProps, DropDownModel> {
  ...
  render() {
    super.render();
    return (
      <TouchableOpacity disabled={this.model.disabled} onPress={this.model.open} style={[DropDownStyles.container]}>
        ...
        <Modal visible={this.model.opened} transparent={true}>
          <TouchableOpacity style={[DropDownStyles.modalContainer]} activeOpacity={1} onPress={this.model.close}>
            <View style={[DropDownStyles.modalContentContainer]}>
              {this.model.list.length === 0 && <Text>No items found</Text>}
              <ScrollView style={BaseStyles.w100} contentContainerStyle={[BaseStyles.w100, BaseStyles.alignCenter]}>
